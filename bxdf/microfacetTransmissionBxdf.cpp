...
Vector3 MicrofacetTransmissionBxdf::SampleGGX(const Vector3 &N, float roughness) const
{
    ...
    // debug...
    if (alpha <= 0.0f || alpha == MathConstant::FLOAT_SMALL_NUMBER)
    {
        return N; // 强制采样 H 就是宏观法线 N
    }
...
Color MicrofacetTransmissionBxdf::sample_f(const Vector3 &wo, Vector3 &wi, float &pdf, const Interaction &interaction) const
{
    ...
    // --- 添加以下调试打印 ---
    // std::cout << "DEBUG: current_etaI = " << current_etaI << ", current_etaT = " << current_etaT << std::endl;
    // std::cout << "DEBUG: local_wo = " << local_wo.x << ", " << local_wo.y << ", " << local_wo.z << std::endl;
    // std::cout << "DEBUG: local_N = " << local_N.x << ", " << local_N.y << ", " << local_N.z << std::endl;
    // std::cout << "DEBUG: H = " << H.x << ", " << H.y << ", " << H.z << std::endl;
    // std::cout << "DEBUG: wi (from refract) = " << wi.x << ", " << wi.y << ", " << wi.z << std::endl;
    // std::cout << "DEBUG: cosThetaO = " << cosThetaO << std::endl;
    // std::cout << "DEBUG: cosThetaI = " << cosThetaI << std::endl;
    // std::cout << "DEBUG: product (cosThetaI * cosThetaO) = " << (cosThetaI * cosThetaO) << std::endl;
    // std::cout << "DEBUG: isTotalReflected = " << isTotalReflected << std::endl;
    // --- 调试打印结束 ---
...
Color MicrofacetTransmissionBxdf::f(const Vector3 &wo, const Vector3 &wi) const
{
    ...
    if (cosThetaO * cosThetaI >= 0)
        return Color(0.0f); // 同半球，不是透射
    if (cosThetaO == 0 || cosThetaI == 0)
        return Color(0.0f); // 掠射角，无贡献
...
float MicrofacetTransmissionBxdf::D(const Vector3 &wh) const
{
...
#if _FIX_BUG_
    if (alpha_ == MathConstant::FLOAT_SMALL_NUMBER)
    {
        // 检查 wh 是否精确等于 (0,0,1)
        if (std::abs(wh.x) < FLT_EPSILON &&
            std::abs(wh.y) < FLT_EPSILON)
        // std::abs(wh.z - 1.0f) < FLT_EPSILON)
        {
            float alpha2 = alpha_ * alpha_;
            if (alpha2 == 0.0f)
                return 0.0f; // 安全检查
            return 1.0f / (MathConstant::PI * alpha2);
        }
        else
        {
            return 0.0f;
    ...
    if (denominator == 0.0f)
    {
        // std::cout << "DEBUG: D = 0.0f" << std::endl;
        return 0.0f;
    }

    float v = alpha2 / denominator;
    // std::cout << "DEBUG: D = " << v << std::endl;
    return v;
}
